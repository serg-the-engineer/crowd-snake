...
- Keep the PR description aligned with the actual `crowd-snake` runtime/deploy
  contract.
...
6. Write/update the PR body explicitly.
   ...
   - If no template exists, use a concise body with:
     - `## Summary`
     - `## Validation`
     - `## Deployment Notes` when deploy/runtime contract changed
   - If PR already exists, refresh the body so it reflects the total branch
     scope, not just the newest commit.
   - Call out any changes to `.env.example`, deploy workflow variables/secrets,
     or `docs/demo-deploy.md` when relevant.
...
# Draft/update a PR body that matches the actual branch scope.
tmp_pr_body=$(mktemp)
cat > "$tmp_pr_body" <<'EOF'
## Summary
- <what changed>
...
- This repo currently has CI around `./scripts/smoke-test.sh`; do not skip that
  gate before publishing branch updates.
